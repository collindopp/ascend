...
interface ActiveSessionViewProps {
  ...
  initialCounts: { dials: number; conversations: number; appointments: number };
}

type EventType = "DIAL" | "CONVERSATION" | "APPOINTMENT";
...
export function ActiveSessionView({ sessionId, leadListName, startedAt, initialCounts }: ActiveSessionViewProps) {
  ...
  const handleTap = useCallback(
    (type: EventType) => {
      const field = type === "DIAL" ? "dials" : type === "CONVERSATION" ? "conversations" : "appointments";
  ...
  function handleUndo() {
    ...
    startTransition(async () => {
      ...
      if (undone) {
        toast.push({ title: `Undid last ${undone.toLowerCase()}`, tone: "default" });
      }
    });
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-3 gap-4 border-y border-border-subtle py-5">
        <MetricDisplay label="Dials" value={formatInt(counts.dials)} size="md" />
      ...
      <Button size="lg" variant="secondary" onClick={() => handleTap("DIAL")} className="w-full">
        Log dial
      </Button>
